...
import { BtnCellGridBrechaComponent } from '../btn-cell-grid-brecha/btn-cell-grid-brecha.component';

@Component({
  selector: 'app-cierra-brechas',
  templateUrl: './cierra-brechas.component.html',
  styleUrls: ['./cierra-brechas.component.css']
})
export class CierraBrechasComponent implements OnInit {
  @ViewChild('run')
  ...
  public frameworkComponents = {
    btnCell: BtnCellGridBrechaComponent
  }
  ...
  columnDefs = [
    {
      headerName: 'Ver Brecha',
      field: 'idEvaluacion',
      width: 75,
      cellRenderer: 'btnCell',
      headerComponentParams: { template: '<i style="margin-left:15px;" class="fas fa-folder-plus"></i>' },
    },
    ...
    { field: "fechaInforme ", type: 'dateColumn', valueFormatter: this.dateFormatter },
  ...
  constructor(private router: Router, private procesoService: ProcesoService, private compartidosService: CompartidosService,
  ...
  ngOnInit(): void {
    let salida = localStorage.getItem("busqueda");
    this.procesoModel=typeof salida === 'string' ? JSON.parse(salida) : new ProcesoIn();
  ...
  buscarProceso() {
    ...
    localStorage.setItem("busqueda",JSON.stringify(this.procesoModel));
    this.procesoService.getProceso(this.procesoModel)
      .then(res => {
        let auxiliar = res.filter((x: any) => x.idEstado > 4 && (x.brechaPct > 0 || x.brechaProt > 0 || x.brechaEI > 0));
        this.resultadoProcesoModel = auxiliar;
        this.rowData = auxiliar;
        this.cargarResumen();
      });
  }
